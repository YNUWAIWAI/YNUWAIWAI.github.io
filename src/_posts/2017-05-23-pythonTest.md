...
# とりあえず書いてみる
Pythonの標準ライブラリにある[unittest](https://docs.python.jp/3/library/unittest.html)を使ってテストを書く例を見せたいと思います．
今回は渡された文字列がURLかどうかを判断する関数のテストを書くことにします．
...
実行結果は以下のようになります．（`${PROJECT_ROOT}`の部分は各自置き換えてください．）
```Markup
F
======================================================================
...
FAILED (failures=1)
```
ここからわかるのは`self.assertTrue(util.is_url(case))`の部分のテストが失敗したことだけで，具体的にどのケースが失敗したのかがわかりません．
...
FAILED (failures=3)
```
`http://example.com`と `http://example.com?q=hoge&p=fuga`と`http://example.com#piyo`のケースでテストが失敗していることがわかりました．
これは`util.py`の`is_url`関数の実装が間違っていることを示しています．

## 実装を修正する

通らないケースを見ると`http://`で始まっている点で共通していることがわかります．
実装をみると
```python
r'^https://[0-9a-zA-Z/:%$&()~.=+\-_]+$'
```
となっており，`https://`で始まるものしか通らないようになっています．
...
FAILED (failures=2)
```
まだ，実装が間違っているようなので，失敗したケースを見ると`?`と`#`が入ってるものが失敗していると予想できます．
...
OK
```
すべてのテストが通ったので`is_url`関数は正しく実装されている可能性が高いということができます．
